...
import cv2
from model5 import get_model
from noise_model import get_noise_model
from utils1 import batch_psnr
...
def get_args():
    ...
    parser.add_argument("--model", type=str, default="srresnet",
                        help="model architecture ('srresnet')")
    parser.add_argument("--weight_file", type=str, required=True,
                        help="trained weight file")
    parser.add_argument("--test_noise_model", type=str, default="rician,25,25",
...
def main():
    ...
    for image_path in image_paths:
        ...
        #psnr = batch_psnr(denoised_image, image, 1.)
        #print("\nPSNR denoised {0:0.2f}dB".format(psnr))
        if args.output_dir:
            cv2.imwrite(str(output_dir.joinpath(image_path.name))[:-4] + "_n.png", noise_image)
            cv2.imwrite(str(output_dir.joinpath(image_path.name))[:-4] + "_d.png", denoised_image)
        else:
